...
//   db.collection("LEARN_USERS").findOne( { email: "vikivikas41@gmail.com" });
  const updateDB =await db.collection("LEARN_USERS").updateOne( { email: body.email }, { $set: { isDeleted: true} } )       
  ...
  const response = {
    ...
    body: JSON.stringify({responseCode: 1000,message:"Deleted Successfully",updateDB,getNewData}),
  };
  return response;
};
