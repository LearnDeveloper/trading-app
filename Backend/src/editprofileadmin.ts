...
//   db.collection("LEARN_USERS").findOne( { email: "vikivikas41@gmail.com" });
  const updateDB =await db.collection("LEARN_USERS").updateOne( { email: body.email }, { $set: { isBanned: body.banned,Phone_Number : body.phoneNumber,full_name:body.fullName, isActive : body.active} } )       
